    ...
    namespace AsynColav4ships
    {
      ...
      struct Task: public DUNE::Control::PathController
      {
        ...
        Inland_Colav(const IMC::EstimatedState& state, const TrackingState& ts)
        {
          ...
          {
            if (M_ship == 4)
              spew("admm_state(%i,:) = [%.2f; %.2f; %.2f|| %.2f;%.2f;%.2f|| %.2f;%.2f;%.2f|| %.2f;%.2f;%.2f]", i,
          ...
          if (colav == true)
          {
            ...
            if (M_ship == 4)
              MPC4_work(&sz_arg, &sz_res, &sz_iw, &sz_w);
            ...
            if (M_ship == 4)
            {
              spew("alpha=[%g; %g, %g, %g]", alpha_admm_casadi[0], alpha_admm_casadi[1],
                   alpha_admm_casadi[2],alpha_admm_casadi[3]);
              spew("chi_ref=[%g; %g; %g; %g]", chi_ref[0], chi_ref[1], chi_ref[2], chi_ref[3]);
              spew("U_ref=[%g; %g; %g; %g]", U_ref_vec[0], U_ref_vec[1], U_ref_vec[2], U_ref_vec[3]);
            }
            if (M_ship == 2)
            {
            ...
            if (M_ship == 4)
            {
              int mem = MPC4_checkout();
              MPC4(arg, res, iw, w, mem);
            }
