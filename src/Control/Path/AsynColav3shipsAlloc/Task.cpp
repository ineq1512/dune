...
namespace Control
{
  namespace Path
  {
    namespace AsynColav3shipsAlloc
    {
      struct Arguments
      {
        double corridor;
      ...
      struct Task: public DUNE::Control::PathController
      {
        ...
        //! Number of feature
        const static int nf = 5;
        //! Number of pair
        const static int nr = 6;
        // Consensus matrix X
        Eigen::VectorXd X_alloc = Eigen::VectorXd::Zero(nf * nr);
        //! Alloc iteration
        int alloc_iter = 0;
        //! Number of new msg alloc
        int alloc_cnt = 0;
        //! Alloc flag
        bool alloc_flag = false;
        //! Msg id list
        int alloc_id_list[10];
        //! New alloc msg flag
        bool new_alloc = true;
        //! Time from last Alloc run
        double alloc_time_now = 0;
        ...
        void
        consume(const IMC::alloc* msg)
        {
          int id = msg->mmsi;
          spew("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX");
          spew("From ship %d", id);
          int iteration = msg->iter;
          spew("iteration= %d,alloc_iter=%d", iteration, alloc_iter);
          spew("alloc flag %d", idExists(alloc_id_list, alloc_cnt, id));
          if ((iteration == alloc_iter) && !idExists(alloc_id_list, alloc_cnt, id))
          {

            IMC::MessageList<IMC::Svec>::const_iterator itr = msg->s_vec_list.begin();
            int n_svec_list = msg->s_vec_list.size();
            int n_svec_value = (*itr)->s_vecs.size();
            int j = 0;
            for (; itr != msg->s_vec_list.end(); itr++)
            {
              int k = 0;
              spew("s_vec id = %i", (*itr)->id);
              IMC::MessageList<IMC::Svec_value>::const_iterator itr2 = (*itr)->s_vecs.begin();
              for (; itr2 != (*itr)->s_vecs.end(); itr2++)
              {
                int pos = id - 1;
                X_alloc(2 * nf * pos + nf * j + k) = (*itr2)->value;
                spew("X_alloc[%i]=%f", 2 * nf * pos + nf * j + k,
                     X_alloc(2 * nf * pos + nf * j + k));
                spew("k=%i,j=%i", k, j);
                k += 1;
              }
              j += 1;
            }
            alloc_id_list[alloc_cnt] = id;
            alloc_cnt += 1;
        ...
        Inland_Colav(const IMC::EstimatedState& state, const TrackingState& ts)
        {
          ...
          if (colav == true)
          {
            ...
            if (M_ship == 3)
            {
              spew("alpha=[%g; %g,%g]", alpha_admm_casadi[0], alpha_admm_casadi[1],
                   alpha_admm_casadi[2]);
              spew("chi_ref=[%g; %g; %g]", chi_ref[0], chi_ref[1], chi_ref[2]);
              spew("U_ref=[%g; %g; %g]", U_ref_vec[0], U_ref_vec[1], U_ref_vec[2]);
            }
            if (M_ship == 2)
            {
        ...
        step(const IMC::EstimatedState& state, const TrackingState& ts)
        {
          ...
          // Task allocation
          const double alloc_now = Clock::get();
          if (alloc_now >= alloc_time_now + 10.0)
          {

            alloc_time_now = alloc_now;
            
            // if ((alloc_flag == true))
            // {
            //   spew("Consensus processing......");
            ...
            // {
            //   spew("Send new message");
            //   IMC::alloc alloc_data;
            //   alloc_data.mmsi = m_args.mmsi;
            //   alloc_data.iter = alloc_iter;
            //   IMC::MessageList<IMC::Svec>* Svec_list = &alloc_data.s_vec_list;

            //   for (int i = 0; i < M_ship - 1; i++)
            //   {
            //     IMC::Svec* Svec = new IMC::Svec;

            //     Svec->id = i + 1;
            //     IMC::MessageList<IMC::Svec_value>* svec_value_list = &(Svec->s_vecs);
            //     for (int j = 0; j < nf; j++)
            //     {
            //       IMC::Svec_value* value = new IMC::Svec_value;
            //       value->value = (double)(j + i);
            //       svec_value_list->push_back(*value);
            //     }
            //     Svec_list->push_back(*Svec);
            //   }
            //   dispatch(alloc_data);
            //   new_alloc = false;
            // }
